"""
SparseFlow Ops/sparse_maxpool2d.py

SparseMaxPool2d module wrapper.

This keeps MaxPool2d on the same SparseFlow runtime contract used by the
...
class SparseMaxPool2d(nn.Module):
    def __init__(
        ...
        self.backend_family = "sparse_maxpool2d"
        self.diag_path = "maxpool2d_sparse"
    ...
    def _ensure_buffers(self, x4d: torch.Tensor):
        from Kernels.maxpool2d import _select_pool_tile_sizes
        import triton
    ...
    def _forward_4d(self, x: torch.Tensor) -> torch.Tensor:
        ...
        try:
            result = sparse_maxpool2d_forward(
                x=x,
        ...
        if self.collect_diag:
            self._last_diag = {
                ...
                "kernel_type": f"maxpool{k_pair_str(self.kernel_size)}/s{k_pair_str(self.stride)}",
    ...
    def extra_repr(self) -> str:
        return (
            ...
            f"dilation={self.dilation}, ceil_mode={self.ceil_mode}, threshold={self.threshold}, "
