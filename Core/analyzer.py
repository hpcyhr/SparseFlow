...
class ReplacementTarget:
    """
    op_type values:
      - "conv2d_3x3"           : standalone sparse 3x3 conv (stride=1 or 2)
      - "conv2d_1x1"           : standalone sparse 1x1 conv (stride=1)
      - "fused_conv3x3_lif"    : fused 3x3 conv + LIF (stride=1 or 2)
      - "fused_conv1x1_lif"    : fused 1x1 conv + LIF
...
class NetworkAnalyzer:
    ...
    Now supports:
      - 1x1, stride=1, groups=1
      - 3x3, stride=1 or 2, padding=1, groups=1
      - keeps min(H, W) >= 7 gatekeeper
    """
    ...
    def analyze(self, model: nn.Module,
                sample_input: Optional[torch.Tensor] = None) -> List[ReplacementTarget]:
        input_shapes = {}
    ...
    def _make_conv_target(self, conv_name, conv_module, spike_name, input_shapes, model=None):
        ...
        if k == (3, 3) and s in ((1, 1), (2, 2)) and p == (1, 1) and g == 1:
            base_op = "conv2d_3x3"
