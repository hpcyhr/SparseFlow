...
def build_spiking_resnet101(
    ...
    model = spiking_resnet.spiking_resnet101(
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Spiking-ResNet101 稀疏度批量统计（全 testloader，单一 T/B 配置）"
    )
    ...
    parser.add_argument(
        "--output_csv",
        type=str,
        default="ALL_resnet101_cifar10_sparsity_summary.csv",
...
def main():
    ...
    model, dev = build_spiking_resnet101(
        pretrained=args.pretrain,
        step_mode="m",
        device=args.device,
    )
    print(f"[INFO] 模型构建完成（ResNet101），device = {dev}")
