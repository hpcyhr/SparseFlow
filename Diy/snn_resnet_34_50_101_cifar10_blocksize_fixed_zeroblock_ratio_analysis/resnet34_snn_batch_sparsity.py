...
def encode_poisson(images: torch.Tensor, T: int, H: int, W: int, device: torch.device):
    """
    images: (B, C, H0, W0) in [0,1]
    返回: (T, B, C, H, W) 的 0/1 spike
    """
    ...
    rate_seq = rate.unsqueeze(0).repeat(T, 1, 1, 1, 1)   # (T,B,C,H,W)

    input_data = torch.bernoulli(rate_seq)               # 0/1 spike
    return input_data
...
class LayerBlockStats:
    def __init__(self, block_sizes: Iterable[int], threshold: float = 1e-6):
        ...
        # name -> {"active":..., "total":..., "block": {b: {"zero":..., "total_blocks":...}}}
        self.stats: Dict[str, Dict[str, Any]] = {}

    def update(self, name: str, spikes: torch.Tensor):
        """
        spikes: (T,B,C,H,W) on CPU
        """
        if spikes.ndim != 5:
            return
    ...
    def finalize_per_layer(self) -> List[Dict[str, Any]]:
        """
        返回每层每个 block 的统计结果列表：
        {layer, block_size, overall_sparsity, zero_block_ratio}
        """
    ...
    def summarize_by_block(rows: List[Dict[str, Any]]):
        """
        按 block_size 汇总：对 layer 维度做 mean。
        """
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Spiking-ResNet34 稀疏度批量统计（全 testloader，单一 T/B 配置）"
    )
    ...
    parser.add_argument(
        "--output_csv",
        type=str,
        default="ALL_resnet34_cifar10_sparsity_summary.csv",
...
def main():
    ...
    # 1. 构建模型
    model, dev = build_spiking_resnet34(
        pretrained=args.pretrain,
        step_mode="m",
        device=args.device,
    )
    print(f"[INFO] 模型构建完成（ResNet34），device = {dev}")
    ...
    def get_hook(name: str):
        def hook(m, i, o):
            # o: (T,B,C,H,W) on device -> 挪到 CPU 再统计
    ...
    # 4. 遍历 testloader，逐 batch 前向 + 累积稀疏度
    model.eval()
