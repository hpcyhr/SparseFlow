...
W=224
NUM_WORKERS=4
DEVICE="cuda:1"          # 想用 A100 改成 cuda 或 cuda:0
BLOCKS="16 32"  
THRESHOLD=1e-6
DATASET="cifar10"
DATA_ROOT="../../data"
...
OUTPUT_CSV="ALL_resnet18_${DATASET}_sparsity_summary.csv"

# 先删掉旧文件，防止 append 到之前的结果
rm -f "${OUTPUT_CSV}"

echo "=== sweep Spiking-ResNet18 稀疏度（全 testloader） ==="
...
for T in "${T_LIST[@]}"; do
  for BATCH_SIZE in "${BATCH_LIST[@]}"; do
    ...
    python3 resnet18_snn_batch_sparsity.py \
      --T "${T}" \
      --H "${H}" \
      --W "${W}" \
