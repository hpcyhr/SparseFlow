"""
SparseFlow Benchmark — Spiking-VGG on CIFAR-10/100
...
用法：
    cd ~/SparseFlow
    python Benchmark/bench_vgg.py --model vgg16_bn --dataset cifar10
    python Benchmark/bench_vgg.py --model vgg19_bn --dataset cifar100 --T 32
    python Benchmark/bench_vgg.py --model vgg11_bn --dataset cifar10 --gpu 2

注意：
    - spikingjelly 的 SpikingVGG 支持 pretrained (ImageNet 权重)
    - VGG 以纯 Sequential (features + classifier) 结构为主，无 residual 分支
    - _bn 后缀版本含 BatchNorm，推荐使用（训练更稳定）
"""
...
def analyze_network(model, sample_input, device):
    ...
    for i, (name, module) in enumerate(module_list):
        ...
        # VGG 是纯顺序结构，搜索窗口可以适当缩小，但保持与 ResNet 一致的 15
...
MODEL_BUILDERS = {
    "vgg11":    spiking_vgg.spiking_vgg11,
    "vgg11_bn": spiking_vgg.spiking_vgg11_bn,
    "vgg13":    spiking_vgg.spiking_vgg13,
    "vgg13_bn": spiking_vgg.spiking_vgg13_bn,
    "vgg16":    spiking_vgg.spiking_vgg16,
    "vgg16_bn": spiking_vgg.spiking_vgg16_bn,
    "vgg19":    spiking_vgg.spiking_vgg19,
    "vgg19_bn": spiking_vgg.spiking_vgg19_bn,
}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="SparseFlow Benchmark — Spiking VGG")
