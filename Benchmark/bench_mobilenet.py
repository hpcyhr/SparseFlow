"""
SparseFlow Benchmark — Spiking-MobileNet on CIFAR-10/100
...
用法：
    cd ~/SparseFlow
    python Benchmark/bench_mobilenet.py --model mobilenet_v2 --dataset cifar10
    python Benchmark/bench_mobilenet.py --model mobilenet_v2 --dataset cifar100 --T 32
    python Benchmark/bench_mobilenet.py --model mobilenet_v2 --dataset cifar10 --gpu 2

注意：
    - spikingjelly 没有内置 spiking_mobilenet 模型，本脚本基于 torchvision 的
      MobileNetV2 手动构造 Spiking 版本：
        1. 加载 torchvision pretrained MobileNetV2 权重
        2. 将所有 ReLU6 替换为 spikingjelly LIFNode
        3. 设置多步模式 (step_mode="m")
    - MobileNetV2 大量使用 depthwise separable conv (groups=C_in)
      当前 SparseFlow 仅对标准 3×3 conv (groups=1) 进行稀疏加速，
      depthwise conv 会被 analyze_network 自动跳过
    - 由于 MobileNetV2 的 3×3 conv 几乎全是 depthwise，实际能被 SparseFlow
      加速的层主要是 1×1 pointwise conv（如果 kernel 支持的话）
"""
...
def verify_layer(feat, conv_module, block, layer_name):
    ...
    Returns:
        max_abs_err, mean_abs_err, max_rel_err, cosine_sim
    """
    feat = feat.contiguous()
...
def analyze_network(model, sample_input, device):
    """分析网络，识别脉冲后继的 Conv2d 层（仅 groups=1）"""
    input_shapes = {}
    hooks = []
    ...
    for i, (name, module) in enumerate(module_list):
        ...
        for j in range(i + 1, min(i + 15, len(module_list))):
            ...
            if isinstance(next_module, nn.Conv2d):
                # ---- 关键：跳过 depthwise conv (groups > 1) ----
                if next_module.groups > 1:
                    # depthwise conv 不是 SparseFlow 当前支持的目标
                    visited.add(next_name)
                    print(f"  [SKIP  ] LIF={name:<30} -> {next_name:<30} "
                          f"(depthwise, groups={next_module.groups})")
                    continue
                ...
                # 支持 3×3 stride=1 pad=1 和 1×1 stride=1 pad=0
                supported = False
                ...
                visited.add(next_name)
                k_str = f"{k[0]}x{k[1]}"
                print(f"  [CONV  ] LIF={name:<30} -> {next_name:<30} "
                      f"k={k_str} H={H} BLOCK_M={compute_block_m(H, W)}")
                continue
...
# =============================================================================
# Spiking MobileNetV2 构建
# =============================================================================

def _replace_relu6_with_lif(model, v_threshold=0.5):
    """
    递归替换模型中所有 ReLU6 / ReLU 为 LIFNode。
    同时处理 nn.Sequential 中的直接替换。
    """
    for name, module in model.named_children():
        if isinstance(module, (nn.ReLU6, nn.ReLU)):
            lif = sj_neuron.LIFNode(
                surrogate_function=sj_neuron.LIFNode().surrogate_function,
                detach_reset=True,
            )
            lif.v_threshold = v_threshold
            setattr(model, name, lif)
        else:
            _replace_relu6_with_lif(module, v_threshold)
...
def build_model(model_name, device, v_threshold=0.5):
    """
    构建 Spiking MobileNetV2：
      1. 加载 torchvision 预训练权重
      2. ReLU6 → LIFNode
      3. 设置多步模式
    """
    ...
    # 替换激活函数
    _replace_relu6_with_lif(model, v_threshold)

    # 设置 spikingjelly 多步模式
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="SparseFlow Benchmark — Spiking MobileNet")
    ...
    print(f"  注意: MobileNetV2 以 depthwise separable conv 为主，")
    print(f"  SparseFlow 当前仅加速 groups=1 的标准 Conv2d")
    ...
    # ---- 分析网络 ----
    print(f"[3/6] 分析网络，识别脉冲后继 Conv2d (groups=1) ...")
    ...
    if not layer_infos:
        print("未找到符合条件的目标层（MobileNetV2 的 3×3 conv 均为 depthwise）。")
        print("提示：可在 Depthwise Conv kernel (P2) 完成后重新运行此 benchmark。")
        return
    print(f"\n共找到 {len(layer_infos)} 个目标 Conv2d 层 (groups=1)\n")
