...
def _sparse_conv1d_kernel(
    x_nlc_ptr, w_kc_ptr, bias_ptr, y_ptr,
    ...
    if tile_cls == TILE_DENSEISH:
        for k in tl.static_range(KS):
            ...
            for cin_base in range(0, C_IN, DENSE_K):
                ...
                x_t = tl.load(x_nlc_ptr + x_addrs,
                              mask=l_ok[:, None] & k_mask[None, :], other=0.0).to(tl.float16)
...
def sparse_conv1d_forward(
    ...
    # [C.7] single popcount + single host sync
    if need_stats:
    ...
    if not launch_all_tiles:
        ...
        if n_active == 0:
            y = torch.zeros((N, C_OUT, L_OUT), device=device, dtype=out_dtype)
            if HAS_BIAS:
                y += bias.to(y.dtype).view(1, C_OUT, 1)
            if return_ms:
                end_evt.record()
                torch.cuda.synchronize(device)
                sparse_ms = start_evt.elapsed_time(end_evt)
    ...
    y = torch.empty((N, C_OUT, L_OUT), device=device, dtype=out_dtype)
    bias_arg = (
        bias.detach().to(y.dtype) if bias is not None
        else torch.empty(1, dtype=y.dtype, device=device)
    )
    if HAS_BIAS:
        y.copy_(bias.to(y.dtype).view(1, C_OUT, 1).expand(N, C_OUT, L_OUT))
    else:
        y.zero_()
