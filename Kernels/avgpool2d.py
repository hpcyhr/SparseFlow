"""
SparseFlow Kernels/avgpool2d.py

Sparse AvgPool2d with metadata-first zero-tile skipping.
        ...
        if oh < H_OUT:
            for bw in range(BW):
                ...
                if ow < W_OUT:
                    ...
                    denom = tl.full([1], KH * KW, dtype=tl.float32)
                    if COUNT_INCLUDE_PAD == 0:
                        denom = valid_count.to(tl.float32)
                    if HAS_DIVISOR_OVERRIDE != 0:
                        denom = tl.full([1], DIVISOR_OVERRIDE, dtype=tl.float32)
...
def sparse_avgpool2d_forward(
    ...
    try:
        _prescan_avgpool2d_kernel[(total_tiles,)](
            x_f16,
            ag_mask_buf,
            tile_class_buf,
            N=N,
    ...
    try:
        _sparse_avgpool2d_kernel[(launch_count,)](
            x_f16,
            y,
            tile_class_buf,
            tile_ids_ptr,
            ...
            HAS_DIVISOR_OVERRIDE=1 if divisor_override is not None else 0,
            DIVISOR_OVERRIDE=int(divisor_override or 0),
